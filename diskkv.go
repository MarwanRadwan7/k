...
// createDB creates a PebbleDB Database in the specified directory.
...
// functions below are utility functions used to manage the current data directory of `Pebble DB`.

// isNewRun function is a utility function that checks
// if the current execution of the program is a new run or a continuation of a previous run.
...
// getNodeDBDirName returns the dir of the node
...
// getNewRandomDBDirName returns new random database dir name
...
// replaceCurrentDBFile is responsible for replacing the current database file with a new version.
...
// getCurrentDBDirName function reads the current database
// directory file from the given directory and returns the corresponding dbdir string.
...
// createNodeDataDir creates a new directory with the given `dir` path and returns an error if any.
...
// cleanupNodeDataDir removes all temporary files in the given `dir` directory and returns an error if any.
...
// queryAppliedIndex retrieves the applied index value from the underlying database
